...
import type { DbEvent, DbTab } from '../../types/database.types';
import { useLocale } from '../../../../core/i18n/index';

const TONE = { info: 'unknown', warning: 'warn', critical: 'crit', success: 'ok' } as const;

/** Dòng thời gian sự kiện database (mất/khôi phục kết nối, cảnh báo, deadlock, migration, thao tác). */
export const DbEventList: React.FC<{ events: DbEvent[] | null; onOpen: (tab: DbTab) => void; emptyText: string }> = ({ events, onOpen, emptyText }) => {
      ...
      {events.map((e) => (
        <li key={e.id} className={`ov-tone-${TONE[e.severity]}`}>
          ...
          <span className="pf-event-body">
            <strong>{t(`db.event.${e.type}`)}</strong>
            <span>{e.message}</span>
          </span>
          {e.tab && (
