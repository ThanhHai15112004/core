...
import type { StorageEvent, StorageTab } from '../../types/storage.types';
import { useLocale } from '../../../../core/i18n/index';

const TONE = { info: 'unknown', warning: 'warn', critical: 'crit', success: 'ok' } as const;

/** Dòng thời gian sự kiện storage (kết nối, cảnh báo, thao tác quản trị). */
export const StorageEventList: React.FC<{ events: StorageEvent[] | null; onOpen: (tab: StorageTab) => void; emptyText: string }> = ({
  events,
  onOpen,
  emptyText,
}) => {
      ...
      {events.map((e) => (
        <li key={e.id} className={`ov-tone-${TONE[e.severity]}`}>
          ...
          <span className="pf-event-body">
            <strong>{t(`storage.event.${e.type}`)}</strong>
            <span>{e.message}</span>
          </span>
          {e.tab && (
