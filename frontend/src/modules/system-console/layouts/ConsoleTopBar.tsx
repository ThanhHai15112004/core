...
export const ConsoleTopBar: React.FC<ConsoleTopBarProps> = ({ currentSection }) => {
  ...
  /** Nhãn của segment con: tên runtime, tab của HTTP Traffic/Database/Cache/Storage/Messaging hoặc thành phần của Performance. */
  const detailLabel = detail
    ? currentSection === 'runtimes'
      ? t(`rt.name.${detail}`)
      : currentSection === 'http-traffic'
        ? t(`tr.tab.${detail}`)
        : currentSection === 'performance' && route.params[1]
      ...
      <nav className="tb-breadcrumb" aria-label="breadcrumb">
        ...
        {detail ? (
          <>
            ...
            <span className="tb-crumb-sep">/</span>
            <span className="tb-crumb-current">{detailLabel}</span>
          </>
        ) : (
